...
void	RobotomyRequestForm::execute(const Bureaucrat &executor) const {
	...
	if (executor.getGrade() > this->getGradeToExec())
		...
		int randomNumber = std::rand(); //ARRUMAR
		//Drilling noises
		std::cout << BOLD << "TRRR TRRR... \nVRRRR VRRRR..." << RESET << std::endl;
		std::cout << "RANDOM NUMBER = " << randomNumber << std::endl;
		if (randomNumber % 2 == 0)
			std::cout << GREEN BOLD << this->getName() << " has been robotomized successfully!" << RESET << std::endl;
		else
			std::cout << RED BOLD << "Fail... " << this->getName() << " has  NOT been robotomized!" << RESET << std::endl;
	}
} 